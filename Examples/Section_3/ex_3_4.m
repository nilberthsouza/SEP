...
if variavel ==1
    disp("Os Condutores São de CAA tipo Drake");
    disp("O Raio Médio Geometrico: Ds = 0.0373");
    disp("A distancia entre a fase A e B é 20 pés");
    disp("A distancia entre a fase B e C é 20 pés");
    disp("A distancia entre a fase C e A é 38 pés");
    disp("A distancia média geometrica: D_eq = 24,8 ");
...
if variavel ==2
    D_ab = input("A distancia entre a fase A e B em pés: ");
    D_bc = input("A distancia entre a fase B e C em pés: ");
    D_ca = input("A distancia entre a fase C e A em pés: ");
    
    condutor = input("Insira o codigo-nome do condutor(dentro de aspas duplas): ");
    indice = find(strcmp(A1.Palavra_Codigo, condutor));

    rmg = A1.RMG_Ds(indice);
    dmg = (D_ab*D_bc*D_ca)^(1/3);

end
L = 2*10^(-7)*log(dmg/rmg);
disp("A indutância é: ");
disp(L);
Xl = 2*pi*60*1609*L;

disp("A reatância indutiva por fase ");
disp(Xl);
