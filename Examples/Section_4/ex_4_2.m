...
if variavel ==2
    ...
    d_a = input("Qual a distancia entre os condutores a e b (Em pes): ");
    d_b = input("Qual a distancia entre os condutores b e c (Em pes): ");
    d_c = input("Qual a distancia entre os condutores c e a (Em pes): ");
...
C_n = (2* pi * 8.85*10^-12)/(log(D_eq/r));

Xd = A3(round(D_eq));

disp1 = sprintf('Cn é igual a %d F/mi', C_n);
disp(disp1);

Xc = 1/(C_n*2*pi*60*1609);
disp2 = sprintf("A Reatancia Capacititva é %d Ohms/mi",Xc);
disp(disp2);

disp("Pelas Tabelas: ")
Xa = (A1.Xa_Capacitiva(indice));
disp("Xa Capacitivo para o condutor é: ");
disp(Xa);


disp3 = sprintf("Da tabela A3 o fator de espaçamento de reatancia capacitiva é para %i pés é %d ohms/mi ",D_eq, Xd);
disp(disp3);

Xc_ = (Xa +Xd )*10^6;

disp3 = sprintf("A reatancia capacitiva é de %d ohms.mi para o neutro",Xc_);


Reatancia_comp = Xc_/175;
disp5  = sprintf("Para um comprimento de 175 milhas a Reatancia capacitiva é de: %d Ohms para o Neutro",Reatancia_comp);

disp(disp5);

% Corrente de carregamento
raiz_de_3 = (3)^(1/3);
I_chg = 2*pi*60*(220000/1.44)*(8.8466*10^(-12))*1609;
disp("Corrente de Carregamento I_chg: ");
disp(I_chg);
